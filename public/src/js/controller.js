class Controller{
	...
	init(selectedSong, songData, autoPlayEnabled, multiplayer, touchEnabled){
		...
		if(multiplayer === 2){
			...
			this.rank = p2.rank || defaultRank
		}else{
			this.snd = multiplayer ? "_p" + p2.player : ""
			this.don = account.loggedIn ? account.don : defaultDon
			this.rank =  account.loggedIn ? account.rank : defaultRank
		}
		if(this.snd === "_p2" && this.objEqual(defaultDon, this.don, defaultRank, this.rank)){
			this.don = {
				body_fill: defaultDon.face_fill,
				face_fill: defaultDon.body_fill
			}
			this.rank = {
				rank_name: defaultRank.rank_name,
				rank_color: defaultRank.rank_color
			}
		}
