class Account{
	...
	accountForm(){
		...
		this.getElement('rankname-hint').innerText = strings.account.customrank.rankName
		this.rankname = this.getElement('rankname')
		this.rankname.placeholder = strings.account.customrank.rankName
		this.rankname.value = account.rank?.rank_name || strings.account.customrank.no_rank
		this.inputForms.push(this.rankname)

		this.customrankColor = this.getElement('customrank-color')
		this.customrankColor.value = account.rank?.rank_color || defaultRank.rank_color
	...
	onSave(event){
		...
		if (!noNameChange && (newRank !== account.rank?.rank_name || rankColor !== account.rank?.rank_color)) {
			promises.push(this.request("account/rank", {
				rank_name: newRank,
